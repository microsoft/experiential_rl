...
from examples.erl_frozenlake.erl_frozenlake_agent import ErlFrozenLakeAgent, build_system_prompt
...
def default_feedback_fn(task: dict[str, Any], trajectory: Trajectory) -> str:
    ...
    last_reward = float(last_step.reward) if last_step else float(trajectory.reward if trajectory.reward is not None else 0.0)
    ...
    def describe_outcome() -> str:
        ...
        return "Episode ended unexpectedly."
    ...
    observations = [str(step.observation) for step in action_steps if step.observation is not None]
    repeated_views = sum(1 for prev, curr in zip(observations, observations[1:]) if prev == curr)
    ...
    if not succeeded and ineffective_moves:
        outcome += f" {ineffective_moves} action(s) had no effect (likely attempted to move out of bounds or into a wall)."
    ...
    if last_step:
        if last_step.done:
            final_state_hint = "in a hole"
        elif max_steps and steps_taken >= max_steps:
            final_state_hint = "still on the board when max steps were reached"
    if final_state_hint:
...
class ErlFrozenLakeStateBuilder:
    """Compose prompt updater context from FrozenLake attempts."""
    ...
    def __call__(self, initial_prompt: str, batch: Sequence[dict[str, Any]], attempts: Sequence[dict[str, Any]], metrics: dict[str, float]) -> str:
        ...
        for idx, attempt in enumerate(attempts[-self.max_examples :], start=1):
            ...
            lines.append(f"Seed: {task.get('seed')} | Size: {task.get('size')} | p: {task.get('p')}")
...
class ErlFrozenLakeWorkflow(Workflow):
    """Reflection-enhanced workflow tailored for the FrozenLake environment."""
    ...
    def __init__(
        ...
        state_builder: ErlFrozenLakeStateBuilder | None = None,
        train_dataset: Sequence[dict[str, Any]] | None = None,
        batch_size: int = 1,
        max_concurrency: int = 1024,
        agent_class: type[ErlFrozenLakeAgent] = ErlFrozenLakeAgent,
        env_class: type[ErlFrozenLakeEnv] = ErlFrozenLakeEnv,
        ...
        **kwargs: Any,
        ...
        self.state_builder = state_builder or ErlFrozenLakeStateBuilder()
