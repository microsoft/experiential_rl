...
from examples.erl_sokoban.erl_sokoban_agent import DEFAULT_SYMBOLS as DEFAULT_SYMBOL_MAP, build_system_prompt
...
def load_tasks(train_examples: list[dict], num_tasks: int, batch_size: int, repeats: int = 1) -> list[dict]:
    """Prepare batched Sokoban tasks for workflow execution."""
...
def build_symbol_map_from_args(args: argparse.Namespace) -> dict[str, str]:
    return {
        ...
        "player_on_goal": args.symbol_player_on_goal,
        "box": args.symbol_box,
        "box_on_goal": args.symbol_box_on_goal,
        "goal": args.symbol_goal,
        "wall": args.symbol_wall,
        "floor": args.symbol_floor,
    }
...
def summarize_results(episodes: list, output_dir: str | None = None) -> None:
    """Summarize Sokoban ERL workflow outcomes and optionally persist logs."""
    if not episodes:
    ...
    results_path = os.path.join(output_dir, f"erl_sokoban_results_{timestamp}.json")
    with open(results_path, "w", encoding="utf-8") as f:
        json.dump([episode.to_dict() for episode in episodes], f, indent=2)
    print(f"\nEpisode logs saved to {results_path}")


async def run_erl_sokoban(args: argparse.Namespace) -> None:
    ...
    engine = AgentWorkflowEngine(
        workflow_cls=ErlSokobanWorkflow,
    ...
    print(f"Prepared {len(tasks)} Sokoban batches (batch size={args.batch_size})")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the ERL workflow on Sokoban tasks.")
    ...
    parser.add_argument("--symbol-player-on-goal", type=str, default=DEFAULT_SYMBOL_MAP["player_on_goal"], help="Symbol representing the player on a goal")
    parser.add_argument("--symbol-box", type=str, default=DEFAULT_SYMBOL_MAP["box"], help="Symbol representing a box")
    parser.add_argument("--symbol-box-on-goal", type=str, default=DEFAULT_SYMBOL_MAP["box_on_goal"], help="Symbol representing a box on a goal")
    parser.add_argument("--symbol-goal", type=str, default=DEFAULT_SYMBOL_MAP["goal"], help="Symbol representing a goal tile")
    parser.add_argument("--symbol-wall", type=str, default=DEFAULT_SYMBOL_MAP["wall"], help="Symbol representing a wall")
    parser.add_argument("--symbol-floor", type=str, default=DEFAULT_SYMBOL_MAP["floor"], help="Symbol representing floor/empty space")
    ...
    parser.add_argument("--max-steps", type=int, default=64, help="Maximum environment steps per Sokoban episode")
...
if __name__ == "__main__":
    ...
    cli_args = parse_args()
    asyncio.run(run_erl_sokoban(cli_args))
