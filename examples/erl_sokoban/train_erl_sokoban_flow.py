...
from examples.erl_sokoban.dataset_utils import ensure_sokoban_dataset
from examples.erl_sokoban.erl_sokoban_agent import DEFAULT_SYMBOLS, ErlSokobanAgent, build_system_prompt
from examples.erl_sokoban.erl_sokoban_env import ErlSokobanEnv
from examples.erl_sokoban.erl_sokoban_flow import ErlSokobanWorkflow
...
def build_symbol_map(config: DictConfig) -> dict[str, str]:
    ...
    env_overrides = {
        "player": os.getenv("ERL_SOKOBAN_SYMBOL_PLAYER"),
        "player_on_goal": os.getenv("ERL_SOKOBAN_SYMBOL_PLAYER_ON_GOAL"),
        "box": os.getenv("ERL_SOKOBAN_SYMBOL_BOX"),
        "box_on_goal": os.getenv("ERL_SOKOBAN_SYMBOL_BOX_ON_GOAL"),
        "goal": os.getenv("ERL_SOKOBAN_SYMBOL_GOAL"),
        "wall": os.getenv("ERL_SOKOBAN_SYMBOL_WALL"),
        "floor": os.getenv("ERL_SOKOBAN_SYMBOL_FLOOR"),
    }
...
def build_initial_prompt(config: DictConfig, symbol_map: dict[str, str]) -> str:
    ...
    literal_env = os.getenv("ERL_SOKOBAN_SYSTEM_PROMPT")
    file_env = os.getenv("ERL_SOKOBAN_SYSTEM_PROMPT_FILE")
...
def build_updater_engine_config(config: DictConfig) -> dict[str, dict[str, object]]:
    ...
    engine_name = _maybe_get(updater_cfg, "engine_name", os.getenv("ERL_SOKOBAN_UPDATER_ENGINE", os.getenv("ERL_UPDATER_ENGINE", "openai")))
    if str(engine_name).lower() == "verl":
        return {}

    default_model = os.getenv("ERL_SOKOBAN_UPDATER_MODEL", os.getenv("ERL_UPDATER_MODEL", "Qwen/Qwen3-4B-Instruct-2507"))
    default_base_url = os.getenv("ERL_SOKOBAN_UPDATER_BASE_URL", os.getenv("ERL_UPDATER_BASE_URL", "http://localhost:30000/v1"))
    default_api_key = os.getenv("ERL_SOKOBAN_UPDATER_API_KEY", os.getenv("ERL_UPDATER_API_KEY", "None"))
    default_max_prompt = int(os.getenv("ERL_SOKOBAN_UPDATER_MAX_PROMPT_LENGTH", os.getenv("ERL_UPDATER_MAX_PROMPT_LENGTH", "4096")))
    default_max_response = int(os.getenv("ERL_SOKOBAN_UPDATER_MAX_RESPONSE_LENGTH", os.getenv("ERL_UPDATER_MAX_RESPONSE_LENGTH", "1024")))
    ...
    temperature_env = os.getenv("ERL_SOKOBAN_UPDATER_TEMPERATURE", os.getenv("ERL_UPDATER_TEMPERATURE"))
    top_p_env = os.getenv("ERL_SOKOBAN_UPDATER_TOP_P", os.getenv("ERL_UPDATER_TOP_P"))
...
def build_updater_sampling_params(config: DictConfig) -> dict[str, float]:
    ...
    temperature = os.getenv("ERL_SOKOBAN_UPDATER_TEMPERATURE", os.getenv("ERL_UPDATER_TEMPERATURE", "0.5"))
    top_p = os.getenv("ERL_SOKOBAN_UPDATER_TOP_P", os.getenv("ERL_UPDATER_TOP_P", "0.9"))
...
def build_solver_settings(config: DictConfig):
    ...
    default_model = os.getenv("ERL_SOKOBAN_SOLVER_MODEL", os.getenv("ERL_SOLVER_MODEL", "Qwen/Qwen2.5-7B-Instruct"))
    default_base_url = os.getenv("ERL_SOKOBAN_SOLVER_BASE_URL", os.getenv("ERL_SOLVER_BASE_URL", "http://localhost:30000/v1"))
    default_api_key = os.getenv("ERL_SOKOBAN_SOLVER_API_KEY", os.getenv("ERL_SOLVER_API_KEY", "None"))
    default_temperature = os.getenv("ERL_SOKOBAN_SOLVER_TEMPERATURE", os.getenv("ERL_SOLVER_TEMPERATURE", "0.6"))
    default_top_p = os.getenv("ERL_SOKOBAN_SOLVER_TOP_P", os.getenv("ERL_SOLVER_TOP_P", "0.95"))
    ...
    default_max_prompt = int(os.getenv("ERL_SOKOBAN_SOLVER_MAX_PROMPT_LENGTH", "4096"))
    default_max_response = int(os.getenv("ERL_SOKOBAN_SOLVER_MAX_RESPONSE_LENGTH", "16384"))
...
def main(config: DictConfig):
    train_dataset = ensure_sokoban_dataset("train")
    val_dataset = ensure_sokoban_dataset("test")

    symbol_map = build_symbol_map(config)
    initial_prompt = build_initial_prompt(config, symbol_map)

    workflow_cfg = getattr(config, "workflow", None)
    default_agent_steps = int(os.getenv("ERL_SOKOBAN_AGENT_MAX_STEPS", "64"))
    default_env_steps = int(os.getenv("ERL_SOKOBAN_ENV_MAX_STEPS", str(default_agent_steps)))
    default_success_threshold = float(os.getenv("ERL_SOKOBAN_SUCCESS_REWARD_THRESHOLD", "1.0"))
    solver_engine_name = _maybe_get(workflow_cfg, "solver_engine_name", os.getenv("ERL_SOKOBAN_SOLVER_ENGINE", "workflow"))
    updater_engine_name = _maybe_get(workflow_cfg, "updater_engine_name", os.getenv("ERL_SOKOBAN_UPDATER_ENGINE", os.getenv("ERL_UPDATER_ENGINE", "openai")))

    solver_tokenizer = None
    solver_sampling_params: dict[str, Any] = {}
    solver_rollout_args: dict[str, Any] = {}
    solver_max_prompt = int(os.getenv("ERL_SOKOBAN_SOLVER_MAX_PROMPT_LENGTH", "4096"))
    solver_max_response = int(os.getenv("ERL_SOKOBAN_SOLVER_MAX_RESPONSE_LENGTH", "16384"))
    ...
    workflow_args = {
        ...
        "batch_size": int(os.getenv("ERL_SOKOBAN_BATCH_SIZE", os.getenv("ERL_BATCH_SIZE", "1"))),
        "max_concurrency": int(os.getenv("ERL_SOKOBAN_MAX_CONCURRENCY", os.getenv("ERL_MAX_CONCURRENCY", "4"))),
        ...
        "no_memory": _get_bool(_maybe_get(workflow_cfg, "no_memory", os.getenv("ERL_SOKOBAN_NO_MEMORY", "false"))),
        "no_reflection": _get_bool(_maybe_get(workflow_cfg, "no_reflection", os.getenv("ERL_SOKOBAN_NO_REFLECTION", "false"))),
        "agent_args": {
    ...
    trainer = AgentTrainer(
        workflow_class=ErlSokobanWorkflow,
