...
async def main():
    ...
    test_dataset = DatasetRegistry.load_dataset("hotpotqa-small", "test")
    results = []
        ...
        for msg in result.get("messages", []):
            ...
            if hasattr(msg, "tool_calls") and msg.tool_calls:
                ...
                if not tool_calls and "function_call" in msg.additional_kwargs:
                    tool_calls = [msg.additional_kwargs["function_call"]]
            if tool_calls:
                print("[tool_calls]")
                for call in tool_calls:
                    ...
                    print(f"  - name: {name}")
                    print(f"    args: {args}")
        for idx, raw in enumerate(result.get("raw_agent_outputs", []), start=1):
            print(f"\nRAW LLM OUTPUT (agent step {idx}):\n{raw}\n")
