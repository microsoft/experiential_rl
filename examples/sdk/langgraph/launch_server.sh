...
# Resolve DATA_DIR relative to this script if it is not absolute
...
# If data is on AzureML mount, copy to local /tmp for stability
if [[ "$DATA_DIR" == /mnt/azureml/* ]]; then
    LOCAL_DATA_DIR="/tmp/prebuilt_indices"
    echo "Data is on AzureML mount. Copying to local disk: $LOCAL_DATA_DIR"
    mkdir -p "$LOCAL_DATA_DIR"
    cp -u "$DATA_DIR/e5_Flat.index" "$LOCAL_DATA_DIR/"
    cp -u "$DATA_DIR/corpus.json" "$LOCAL_DATA_DIR/"
