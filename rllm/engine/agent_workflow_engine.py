...
class AgentWorkflowEngine:
    ...
    def transform_results_for_verl(self, episodes: list[Episode], task_ids: np.ndarray) -> "DataProto":
        ...
        adv_estimators = []
        def _get_adv_estimator(step_info: dict | None, traj_info: dict | None):
            if step_info is not None and "adv_estimator" in step_info:
                return step_info.get("adv_estimator")
            if traj_info is not None:
                return traj_info.get("adv_estimator")
            return None
        ...
        for i, episode in enumerate(episodes):
            ...
            for trajectory in episode.trajectories:
                ...
                if not self.config.rllm.stepwise_advantage.enable:
                    ...
                    adv_estimators.append(_get_adv_estimator(trajectory.steps[-1].info if trajectory.steps else None, trajectory.info))
                    n_steps = 1
                    ...
                    for step_idx, step in enumerate(trajectory.steps):
                        ...
                        adv_estimators.append(_get_adv_estimator(step.info, trajectory.info))
        ...
        non_tensors = {
            ...
            "adv_estimator": np.array(adv_estimators, dtype=object),
        }
