...
class VerlEngine(RolloutEngine):
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        # Guard against exceeding model context window in async vLLM server.
        # Server computes max_tokens = max_model_len - len(prompt_ids) and raises if <= 0.
        max_model_len = self.config.actor_rollout_ref.rollout.get("max_model_len", None)
        if max_model_len is None:
            max_model_len = self.max_prompt_length + self.max_response_length
        if prompt_length >= int(max_model_len):
        ...
        try:
            ...
            # Convert vLLM argument validation into a normal workflow termination
            # so one bad sample does not crash the full training run.
            if "max_tokens must be at least 1" in str(e):
                raise TerminationEvent(TerminationReason.MAX_RESPONSE_LENGTH_EXCEEDED) from e
            raise
