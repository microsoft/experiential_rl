...
class AgentWorkflowPPOTrainer(RayPPOTrainer):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        batch = self._compute_advantage_with_overrides(
                            batch,
                            default_estimator=self.config.algorithm.adv_estimator,
    ...
    def _compute_advantage_with_overrides(
        self,
        batch,
        default_estimator: AdvantageEstimator,
        gamma: float,
        lam: float,
        num_repeat: int,
        ...
        """Compute advantages, allowing per-sample adv_estimator overrides."""
        overrides = batch.non_tensor_batch.get("adv_estimator", None)

        def to_adv(est, *, fallback=None):
            if isinstance(est, AdvantageEstimator):
                return est
            try:
                return AdvantageEstimator(est)
            except Exception:
                if fallback is not None:
                    return fallback
                raise

        base_estimator = to_adv(default_estimator, fallback=default_estimator)
        if overrides is None:
            return compute_advantage(
                batch,
                adv_estimator=base_estimator,
                gamma=gamma,
                lam=lam,
        ...
        def resolve(est):
            ...
            try:
                return to_adv(est, fallback=base_estimator)
            except Exception:
                return base_estimator

        resolved = [resolve(est) for est in overrides]
        unique_estimators = list(dict.fromkeys(resolved))

        advantages = torch.zeros_like(batch.batch["response_mask"], dtype=torch.float32)
        returns = torch.zeros_like(batch.batch["response_mask"], dtype=torch.float32)

        for est in unique_estimators:
            idxs = [i for i, choice in enumerate(resolved) if choice == est]
            if len(idxs) == 0:
                continue
            sub_batch = batch.select_idxs(idxs)
            sub_batch = compute_advantage(
                sub_batch,
                adv_estimator=est,
                gamma=gamma,
                lam=lam,
            ...
            idx_tensor = torch.as_tensor(idxs, device=advantages.device)
            advantages[idx_tensor] = sub_batch.batch["advantages"]
            returns[idx_tensor] = sub_batch.batch["returns"]
