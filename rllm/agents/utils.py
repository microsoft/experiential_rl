...
from rllm.parser import ChatTemplateParser, OLMoChatTemplateParser
...
def convert_messages_to_tokens_and_masks(messages: list[dict[str, str]], tokenizer: PreTrainedTokenizerBase, parser: ChatTemplateParser, contains_first_msg=False, contains_generation_msg=False):
    ...
    def _convert_message_to_tokens_and_masks(msg, first_msg=False, generation_msg=False, is_last_msg=False):
        parse_kwargs = {}
        if isinstance(parser, OLMoChatTemplateParser):
            parse_kwargs["use_eos"] = is_last_msg and not generation_msg

        msg_text = parser.parse([msg], add_generation_prompt=generation_msg, is_first_msg=first_msg, **parse_kwargs)
